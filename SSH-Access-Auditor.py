...
"""
    ...
    parser.add_argument(
        ...
        "--accepted",
        action="store_true",
        help="在屏幕显示认证成功的登录记录及统计信息",
    )
    parser.add_argument(
        "-f",
        "--failed",
        action="store_true",
        help="在屏幕显示认证失败的登录尝试及统计信息",
    )
    ...
    parser.add_argument(
        ...
        "--key",
        action="store_true",
        help="仅在屏幕输出中过滤基于公钥 (publicKey) 的认证",
    )
    parser.add_argument(
        "-p",
        "--password",
        action="store_true",
        help="仅在屏幕输出中过滤基于密码 (password) 的认证",
    )
    parser.add_argument(
        "-P",
        "--pam",
        action="store_true",
        help="仅在屏幕输出中过滤基于键盘交互/PAM的认证",
    )

    # 时间过滤参数
    parser.add_argument(
        "-t", "--time", dest="exact_date", help='按指定日期筛选 (例如: -t "12/23")'
    )
    parser.add_argument(
        "-ts",
        "--time-start",
        dest="time_start",
        help='按起始时间筛选 (例如: -ts "12/23 03:00:00")',
    )
    parser.add_argument(
        "-te",
        "--time-end",
        dest="time_end",
        help='按结束时间筛选 (例如: -te "12/23 23:00:00")',
    )
    parser.add_argument(
        "-tr",
        "--time-recent",
        dest="time_recent",
        type=int,
        help="显示最近 X 天的记录 (例如: -tr 7 显示近7天)",
    )
...
def main():
    ...
    # 预处理时间条件
    target_date = (
        parse_user_time(args.exact_date, is_exact_date=True)
        if args.exact_date
        else None
    )
    ...
    try:
        with open(args.logfile, "r", encoding="utf-8", errors="ignore") as file:
            for line in file:
                ...
                if target_date:
                    if (
                        log_time.month != target_date.month
                        or log_time.day != target_date.day
                    ):
                        continue
    ...
    if args.accepted:
        ...
        filter_methods = []
        if args.key:
            filter_methods.append("publickey")
        if args.password:
            filter_methods.append("password")
        if args.pam:
            filter_methods.append("keyboard-interactive/pam")
        ...
        display_acc_events = [
            ...
            if not filter_methods or e["method"] in filter_methods
        ]
    ...
    if args.failed:
        ...
        for d_key in sorted(date_ip_counts.keys()):
            ...
            # 针对当天该IP失败次数进行降序排序
            sorted_ips = sorted(
                date_ip_counts[d_key].items(), key=lambda item: item[1], reverse=True
            )
    ...
    if args.outfile:
        ...
        # 2. 计算存在爆破行为的 IP (> 10 次)
        brute_force_ips = {
            ip: count for ip, count in ip_total_fails.items() if count > 10
        }
        brute_force_str_list = [
            f"{ip}（{count}次）"
            for ip, count in sorted(
                brute_force_ips.items(), key=lambda x: x[1], reverse=True
            )
        ]
        ...
        # 4. 提取所有出现的日期，便于时间线输出
        all_dates = set(
            [e["date"] for e in accepted_events] + [e["date"] for e in failed_events]
        )
        ...
        for current_date in sorted(all_dates, key=sort_date_str):
            ...
            if current_date in fail_by_date:
                ...
                for e in fail_by_date[current_date]:
                    day_fails[e["ip"]] += 1
                sorted_day_fails = sorted(
                    day_fails.items(), key=lambda item: item[1], reverse=True
                )
